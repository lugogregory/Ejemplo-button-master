...
export const ButtonBaseSpan = styled.span`
  display: flex;
  align-items: center;
...
const WrapperSpan = styled(ButtonBaseSpan) <{
  fontColor: string,
...
type Props = {
  ...
  disabled?: boolean,
  upload?: boolean, //true --> button opload span
};
...
const Button = ({
  ...
  if (upload) {
    return (
      <WrapperSpan onMouseEnter={() => onEnter()} onMouseLeave={() => onLeave()}
        onClick={onClick}
        fontColor={fontColor}
      ...
      >
        {
          Icon ? <Icon style={{
            fill: hover ? fillHoverColor !== '' ? fontHoverColor : fillColor : fillColor,
